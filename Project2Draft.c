...
void LEDCheck(void){	
	// Set all three LEDs to be on, then blink three times;
	
	GPIO_PORTB_DATA_R |= 0x0F; // Set (turn on PB0 PB1 PB2 PB3)
	Delay(1);
	GPIO_PORTB_DATA_R ^= 0x0F; Delay(1); //GPIO_PORTB_DATA_R |= 0x0E; // Off and on
	GPIO_PORTB_DATA_R ^= 0x0F; Delay(1); //GPIO_PORTB_DATA_R |= 0x0E; // Off and on
	GPIO_PORTB_DATA_R ^= 0x0F; Delay(1); //GPIO_PORTB_DATA_R |= 0x0E; // Off and on
	GPIO_PORTB_DATA_R ^= 0x0F; Delay(1);
	GPIO_PORTB_DATA_R ^= 0x0F;     	     // Clear (turn off PB1 PB2 PB3)
}
...
{	
  ...
  In &= 0xF0; 				// Clear bits that we don't care about
	//In = GPIO_PORTB_DATA_R;	// Collect and shift external switch states
	if (In == 0x0F) 						      		// If all phototransistors are UNmasked
	{
		GPIO_PORTF_DATA_R |= (0x08);     		// Turn on green internal LED
		//return;
	}
	else if (In == 0x00)									// If all phototransistors are masked
	{
		GPIO_PORTF_DATA_R |= (0x02);     		// Turn on red internal LED
		//return;
	}
	else
	{
		GPIO_PORTF_DATA_R &= ~(0x0A);     	// Turn off both internal LEDs
	}
	state = In;
}
